...
the two pairs of `.` in the southwest and southeast (marked `I` below).  
...
some of which are in the loop (`I`) and some of which are outside it (`O`):
...
Here's another example with many bits of junk pipe lying around  
